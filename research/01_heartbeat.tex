...
El circuito fue ensamblado sobre protoboard siguiendo el esquemático de la Figura~\ref{fig:esquema_v1}.
La alimentación se suministró desde el bus de \SI{5}{\volt} del programador PICKit2 vía ICSP,
sin fuente externa adicional en esta iteración.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.72\textwidth]{HeartbeatSuperior.jpg}
    \caption{Montaje físico del DomoNode V1 en protoboard — Iteración 01.}
    \label{fig:protoboard}
\end{figure}

La validación en hardware real confirmó el comportamiento simulado: el LED conectado a \textbf{RD7 (Pin~30)}
oscila a \SI{1}{\hertz} con un \textit{duty cycle} del 50\%, precedido por la ráfaga de inicio
que distingue el arranque de la ejecución normal.
...
Para la validación funcional se utilizó \textbf{PICSimLab v0.9.2} emulando una placa \textit{McLab2}. Durante las pruebas iniciales, se identificó una limitación en el paso de tiempo del simulador al operar por debajo de \SI{1}{\mega\hertz}. 

Para garantizar una correlación 1:1 con el tiempo real en el osciloscopio virtual, se optó por una \textbf{estrategia de ajuste de validación}:
\begin{itemize}
